...
def apply_coupons(cart, coupons)
  coupons.each do |coupon|
    ...
    if cart[item] 
      if cart[item][:count] >= coupon[:num] && !cart[new_item]
      cart[new_item] = {price: coupon[:cost] / coupon[:num], clearance: cart[item][:clearance], count: coupon[:num]}
      cart[item][:count] -= coupon[:num]
      elsif cart[item][:count] >= coupon[:num] && cart[new_item]
